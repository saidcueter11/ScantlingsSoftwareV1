...
import { useScantlingsContext } from './ScantlingsContext'

const PressureContext = createContext({})

export const PressureProvider = ({ children }: { children: React.ReactNode }) => {
  const { mLDC, LWL, BC, category, V, B04, context, material, z, l, s, lu, b, hB, x, hp, hs, height, base, areaPerforation } = useScantlingsContext()

  const [PBMDBASE, setPBMDBASE] = useState(0)
  const [PBMPBASE, setPBMPBASE] = useState(0)
  const [PBMMIN, setPBMMIN] = useState(0)
  const [PSMMIN, setPSMMIN] = useState(0)
  const [PDMBASE, setPDMBASE] = useState(0)
  const PDMMIN = 5

  const [bottomPressure, setBottomPressure] = useState(0)
  const [sideTransomPressure, setSideTransomPressure] = useState(0)
  const [deckPressure, setDeckPressure] = useState(0)
  const [superstructuresDeckhousesPressure, setSuperstructuresDeckhousesPressure] = useState(0)
  const [watertightBulkheadsPressure, setWatertightBulkheadsPressure] = useState(0)
  const [integralTankBulkheadsPressure, setIntegralTankBulkheadsPressure] = useState(0)
  const [washPlatesPressure, setWashPlatesPressure] = useState(0)
  const [collisionBulkheadsPressure, setCollisionBulkheadsPressure] = useState(0)
  ...
  function dynamicLoadNCG () {
    ...
    if (nCG1 > 3) {
      ...
      nCG = nCG1
    } // # Si nCG_1 es menor o igual a 3, lo usamos directamente
  ...
  function calculateSuperstructuresDeckhousesPressure (): number {
    const kAR = areaPressureReductionKAR()
    const kDC = designCategoryKDC()
    const kSUP = 1
...
export const usePressureContext = () => {
  return useContext(PressureContext)
}
